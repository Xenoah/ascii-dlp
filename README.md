...
- 端末サイズに合わせて縦横比を自動調整
- 音声再生、一時停止、シークに対応
- `pip install`だけでFFmpeg・音声ライブラリを自動導入（PATH設定不要）
...
FFmpeg、ffprobe、yt-dlp、音声再生ライブラリは下記コマンドで一緒に入ります。FFmpegの手動インストールやPATH設定は不要です。すでにシステムのFFmpegがある場合は、そちらを優先して使います。

## インストール

GitHub Releaseのwheelを直接指定できます。リポジトリのcloneは不要で、必要なPythonパッケージとOS別FFmpegバイナリも一緒にインストールされます。FFmpegを含むため、初回は大きめのダウンロードになります。
...
仮想環境や`pipx`を利用している場合も、同じwheel URLをインストール対象に指定できます。
...
インストールが中断された可能性があります。上記のGitHub wheel URLを指定し直してアップグレードしてください。外部FFmpegを使いたい場合だけ、場所を直接指定できます。
...
`ascii-dlp doctor`で`miniaudio`を確認してください。Bluetooth機器などで映像とのずれがある場合は補正できます。
...
本プロジェクトはMIT Licenseです。`yt-dlp`、FFmpegバイナリ、`miniaudio`はそれぞれのライセンスで提供される外部依存関係です。詳細は`THIRD_PARTY_NOTICES.md`を参照してください。
